...
<template>
  <Head title="Public Files" />

  <AuthenticatedLayout>
    <template #header>
      <h2 class="text-2xl font-bold leading-tight text-gray-900">🌐 Public Files</h2>
    </template>
    ...
    <div class="py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-col sm:flex-row gap-4 mb-6">
        <input
          ...
          class="flex-1 border border-gray-300 rounded-lg px-4 py-2.5 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500"
        />
        <Link
      ...
      <div v-if="filtered.length === 0" class="bg-white shadow rounded-lg p-12 text-center">
        ...
        <p class="text-gray-600 font-medium">No public files yet</p>
        <p class="text-gray-500 text-sm mt-2">No files shared publicly</p>
      </div>

      <!-- File Table -->
      <div v-else class="bg-white shadow rounded-lg overflow-hidden">
        <table class="w-full text-sm">
          <thead class="bg-green-50 text-gray-700 uppercase text-xs font-semibold">
            <tr>
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="file in filtered" :key="file.id" class="hover:bg-green-50 transition duration-150 ease-in-out">
              <td class="px-6 py-3">
              ...
              <td class="px-6 py-3">
                <span class="px-3 py-1 rounded-full text-xs font-medium inline-block bg-green-100 text-green-800">
                  🌐 Public
                </span>
              </td>
